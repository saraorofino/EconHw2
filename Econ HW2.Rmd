...
kable(cs_high_table, col.names = c("Tax Amount", "High Income Consumer Welfare"), digits = 2) %>% 
...
kable(cs_low_table, col.names = c("Tax Amount", "Low Income Consumer Welfare"), digits = 2) %>% 
  kable_styling(bootstrap_options = c("striped", "condensed", "bordered"), full_width = FALSE)
...
kable(ps_table, col.names = c("Tax Amount", "Producer Welfare")) %>% 
  kable_styling(bootstrap_options = c("striped", "condensed", "bordered"), full_width = FALSE)
...
tax_table <- tax_scenarios %>%
  ...
  select(amt_tax, rev_tax,cs_low_change, cs_high_change, ps_change) %>% 
  filter(amt_tax == 0.25 | amt_tax == 1.75 | amt_tax == 3.25 | amt_tax == 5.00)
  

taxtable_final <- kable(tax_table, col.names = c("Tax Amount (USD)", "Tax Revenue (USD)", "Welfare Change Low Income (%)", "Welfare Change High Income (%)", "Welfare Change Producers (%)"), digits = 3) %>% 
...
env_damage_final <- kable(env_damage_table, digits = 3) %>% 
  kable_styling(bootstrap_options = c("striped", "bordered", "condensed"), full_width = F)
